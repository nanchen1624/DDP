...
G1=zeros(T,NX,T);%X_t
G2=zeros(T,NR,T);%R_t
H1=zeros(T,NX,NX,T);%X_t^2
H2=zeros(T,NR,NR,T);%R_t^2
H3=zeros(T,NX,NR,T);%X_tR_t
K1=zeros(T,NX,NR,T);%X_tR_t^2
K2=zeros(T,NX,NR,T);%X_t^2R_t^2
K3=zeros(T,NX,T);%X_t^4
K33=zeros(T,T);%X_t^2X_t^2
K4=zeros(T,NR,NR,T);%R_t^2R^2
K5=zeros(T,NR,NR,T);%R_t*R_t^0.5
K6=zeros(T,NX,T);%X_t^3
K7=zeros(T,NR,T);%R_t^3
K8=zeros(T,NX,NR,T);%X_t^2R_t
for i=1:T
    H2(i,:,:,T)=A;
    H3(i,:,:,T)=B';
    K5(i,:,:,T)=E;
...
M=K; % lower bound sample path
J_low=zeros(T,M);
u_low=zeros(T,NR,T,M);

% generate state
Eta_low=zeros(T,NX,T,M);
X_low=zeros(T,NX,T,M);%information
R_low=zeros(T,NR,T,M);% remaining stock
R_low(:,:,1,:)=ones(T,NR,M)*R1;

for i=1:T
    for t=1:T-1
        Eta_low(i,:,t+1,:)=mvnrnd(zeros(1,NX),sigma_eta,M)';
        X_low(i,:,t+1,:)=C*squeeze(X_low(i,:,t,:))+squeeze(Eta_low(i,:,t+1,:));
        %S_temp=R1/T*2*rand(M,1);
        %R(i,t+1,:)=max(squeeze(R(i,t,:))-S_temp,0);
        R(i,:,t+1,:)=R1*rand(NR,M);
    end
end
for i=1:T
    for t=1:T-1
...
% G2_low=zeros(T,NR,T);%R_t
% H1_low=zeros(T,NX,NX,T);%X_t^2
% H2_low=zeros(T,NR,NR,T);%R_t^2
% H3_low=zeros(T,NX,NR,T);%X_tR_t
% K1_low=zeros(T,NX,NR,T);%X_tR_t^2
% K2_low=zeros(T,NX,NR,T);%X_t^2R_t^2
% K4_low=zeros(T,NR,NR,T);%R_t^2R^2
% K5_low=zeros(T,NR,NR,T);%R_t*R_t^0.5
% K6_low=zeros(T,NX,T);%X_t^3
% K7_low=zeros(T,NR,T);%R_t^3
% K8_low=zeros(T,NX,NR,T);%X_t^2R_t
for i=1:T-1
    for t=1:T-1
        parfor n=1:M
...
%% initial state from linear program approximation 
u = zeros(NR, T, K);
% options = optimset('Algorithm','sqp','Display','off');
i=1
for t = T-1:-1:1
%     i=t
    if t>1
        ...
        Coff_temp = regress(squeeze(J_low(t,:))',M_temp);
        F(i,t)=Coff_temp(1);
