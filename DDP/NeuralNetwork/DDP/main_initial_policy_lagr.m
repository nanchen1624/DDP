...
% This is the stock acquisition from Andrew Lo and D. Bertsimas (1998)
...
%% problem setting
K=5*10^4;
% load initial guess value from Lagrange method
fname_input = sprintf('../results/data/generate/initial_value/sample_data_%d.mat', K);
load(fname_input);
iteration=4;
% state
...
i=1
iteration_time = zeros(iteration,2);
iteration_t1 = clock;
...
%         f = sum(M_temp)/K;
%         Coff_temp = linprog(f',M_temp, squeeze(J(i,t,:)));
        ...
        W_initial(i)=mean(J(i,t,:))
        W_varinitial(i)=var(J(i,t,:))
    end
end
iteration_time(i, 1) = etime(clock, iteration_t1);
%% problem solving
% i t k
...
for i=2:iteration+1
    ...
    iteration_time(i, 1) = etime(clock, iteration_t);
end
%% compute upper bound
...
for i=2:iteration+1
    ...
    iteration_time(i, 2) = etime(clock, iteration_t);   
end
Total_time=etime(clock,t1);
fname_output = sprintf('result/K_%d.mat', K);
save(fname_output)